...
def calculate_psi(expected: np.ndarray, actual: np.ndarray, num_bins: int = 10) -> Tuple[float, pd.DataFrame]:
    """
    ...
    # Get bin edges using percentiles of expected population
    percentiles = np.linspace(0, 100, num_bins + 1)
    bin_edges = np.percentile(expected, percentiles)
    # Adjust boundaries slightly to include min and max
    bin_edges[0] -= 1e-5
    bin_edges[-1] += 1e-5
    
    # Bucket the populations
    ...
    # Avoid zero division and log(0) issues by replacing 0 with tiny value
    ...
    bin_labels = [f"Bin {i+1} ({bin_edges[i]:.2f} to {bin_edges[i+1]:.2f})" for i in range(num_bins)]
...
def assess_data_health(df: pd.DataFrame, numerical_cols: list) -> Dict[str, Any]:
    ...
    for col in numerical_cols:
        if col in df.columns:
            ...
            total_numeric_values += len(df[col])
            
    outlier_rate = (total_outliers / total_numeric_values * 100) if total_numeric_values > 0 else 0
    
    # Simple overall score between 0 and 100
    # Penalty: average missing percentage and outlier rate
